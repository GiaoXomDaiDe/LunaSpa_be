import { GetAllBranchesOptions } from '~/models/request/Branches.requests'
...
export function buildBranchesPipeline(options: GetAllBranchesOptions) {
  const defaultOptions: Partial<Required<GetAllBranchesOptions>> = {
    limit: 10,
    page: 1,
    search: ''
  }
  ...
  if (!_options.isAdmin) {
    match.status = { $ne: BranchStatus.INACTIVE }
  }
  if (options.status) {
    match.status = options.status === 'active' ? BranchStatus.ACTIVE : BranchStatus.INACTIVE
  } else {
    match.status = BranchStatus.ACTIVE
  }
  if (_options.min_rating) {
    match.rating = { ...(match.rating || {}), $gte: _options.min_rating }
  }
  if (_options.max_rating) {
    match.rating = { ...(match.rating || {}), $lte: _options.max_rating }
  }
    ...
    $lookup: {
      ...
      pipeline: [
        ...
        },
        {
          $lookup: {
            from: 'branch_services',
            let: { service_id: '$_id', branchServices: '$$branchServices' },
            pipeline: [
              {
                $match: {
                  $expr: {
            ...
            pipeline: [
              {
                $match: {
                  $expr: {
                    $and: [{ $eq: ['$product_id', '$$product_id'] }, { $eq: ['$branch_id', '$$branchId'] }]
                  }
                }
              }
            ],
